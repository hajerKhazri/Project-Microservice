...
export class UsersComponent implements OnInit {
  ...
  myApplicationsProjects: Project[] = [];
  myApplicationsLoading = false;
  withdrawingId?: number;

  get myApplicationsSorted(): Candidature[] {
    return [...this.myApplications].sort((a, b) => {
      const order: Record<string, number> = { PENDING: 0, ACCEPTED: 1, REJECTED: 2, REJECTED_PROJECT_CLOSED: 3 };
      return (order[a.status ?? ''] ?? 9) - (order[b.status ?? ''] ?? 9);
    });
  }

  projectTitleFor(projectId: number): string {
    return this.myApplicationsProjects.find((p) => p.id === projectId)?.title ?? `Project #${projectId}`;
  }
  ...
  applicationStatusClass(status?: string): string {
    switch (status) {
      case 'ACCEPTED': return 'app-accepted';
      case 'REJECTED': return 'app-rejected';
      case 'REJECTED_PROJECT_CLOSED': return 'app-closed';
      default: return 'app-pending';
    }
  }

  withdrawApplication(c: Candidature): void {
    if (!c.id || c.status !== 'PENDING') return;
    if (!window.confirm(`Withdraw application for "${this.projectTitleFor(c.projectId)}"?`)) return;
    this.withdrawingId = c.id;
    this.candidaturesService.delete(c.id)
      .pipe(finalize(() => { this.withdrawingId = undefined; }))
      .subscribe({
        next: () => {
          this.myApplications = this.myApplications.filter((a) => a.id !== c.id);
          this.successMessage = 'Application withdrawn successfully.';
        },
        error: () => {
          this.errorMessage = 'Failed to withdraw the application.';
        }
      });
  }

  loadMyApplications(): void {
    if (!this.isCurrentUserFreelancer) return;
    ...
    this.candidaturesService.getAll()
      .pipe(finalize(() => { this.myApplicationsLoading = false; }))
      .subscribe({
        next: (all) => {
          // Filter to only this user's applications by email
          this.myApplications = (all ?? []).filter(
            (c) => c.email?.toLowerCase() === user.email?.toLowerCase()
          );
        },
        error: () => { this.myApplications = []; }
      });

    this.projectsService.getAll().subscribe({
      next: (p) => { this.myApplicationsProjects = p ?? []; },
      error: () => { this.myApplicationsProjects = []; }
    });
  }
  // ─────────────────────────────────────────────────────────────────
  ...
  signUp(signUpForm: NgForm): void {
    ...
    this.usersService.create(payload)
      ...
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          this.errorMessage = this.extractErrorMessage(error, 'Sign up failed. Check the gateway and the Django user-service.');
        }
      });
  }
  ...
  loadUsers(): void {
    ...
    this.usersService.getAll()
      ...
      .subscribe({
        ...
        error: () => {
          this.errorMessage = 'Cannot load users. Check the gateway route and the Django user-service.';
        }
      });
  }
  ...
  saveUser(userForm: NgForm): void {
    ...
    request
      ...
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          this.errorMessage = this.extractErrorMessage(error, 'Save failed. Check the gateway route and the Django user-service.');
        }
      });
  }
  ...
  deleteUser(user: AppUser): void {
    ...
    this.usersService.delete(user.id)
      ...
      .subscribe({
        ...
        error: (error: HttpErrorResponse) => {
          this.errorMessage = this.extractErrorMessage(error, 'Delete failed. Check the gateway route and the Django user-service.');
        }
      });
  }
  ...
  private extractErrorMessage(error: HttpErrorResponse, fallbackMessage: string): string {
    if (error.status === 0) {
      return 'The browser could not complete the request to the user API. Check that the gateway on port 8091 is running and restart it if CORS headers were duplicated.';
    }
    ...
    if (error.error && typeof error.error === 'object') {
      const firstEntry = Object.entries(error.error).find(([field]) =>
        !['timestamp', 'status', 'error', 'path', 'trace'].includes(field)
      );
